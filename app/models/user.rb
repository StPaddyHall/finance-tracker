class User < ActiveRecord::Base
  ...
  # search paramter to check if you are friends with that person
  ...
  def except_current_user(users)
    # reject users that matches the id of the current user
    users.reject { |user| user.id == self.id }
  end
  
  # this is the search function that will do the work (this is a class level method)
  def self.search(param)
    # if param is blank, return nothing
